...
resource "digitalocean_firewall" "firewall_cn" {
  ...
  droplet_ids = [digitalocean_droplet.vm_cn1.id, digitalocean_droplet.vm_cn2.id]

  inbound_rule {
    protocol   = "tcp"
    port_range = "443"
    #Esta rule foi criada para fazer o acesso somente pela VM do FrontEnd
    source_addresses = [digitalocean_droplet.vm_frontend2.ipv4_address, digitalocean_droplet.vm_frontend1.ipv4_address]
  }
  ...
  inbound_rule {
    ...
    port_range = "80"
    #Esta rule foi criada para fazer o acesso somente pela VM do FrontEnd
    #source_addresses = [digitalocean_droplet.vm_cn1.ipv4_address]
    source_addresses = [digitalocean_droplet.vm_frontend2.ipv4_address, digitalocean_droplet.vm_frontend1.ipv4_address]
  }
